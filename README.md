...
A `📥 Garage61` button appears next to each driver's row when the circuit has an `iracing_track_id` set. It opens an import panel that fetches the driver's laps from Garage61 for that circuit and lets you apply a lap time and fuel consumption directly into the performance form. Filters: condition (dry/wet), session type (P/Q/R), same car, fuel level range (min/max L), date range with presets (7j / 30j / 3 mois). Columns: condition, lap time, fuel used, fuel level, track temp, date, car, session.
