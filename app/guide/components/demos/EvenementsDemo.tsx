          ...
          <div key={ev.name} style={{ display: "flex", alignItems: "center", gap: "1rem", padding: "0.75rem 1rem", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", gap: "0.35rem", marginBottom: "0.25rem", flexWrap: "wrap", alignItems: "center" }}>
                {ev.type === "special" && (
                  <span style={{ fontSize: "0.6rem", fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", padding: "0.1rem 0.4rem", background: "rgba(255,170,0,0.1)", border: "1px solid #ffaa00", borderRadius: "2px", color: "#ffaa00" }}>Spécial</span>
                )}
                {ev.type === "championnat" && (
                  <>
                    <span style={{ fontSize: "0.6rem", fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", padding: "0.1rem 0.4rem", background: "rgba(160,100,240,0.1)", border: "1px solid #a064f0", borderRadius: "2px", color: "#a064f0" }}>Championnat</span>
                    <span style={{ fontSize: "0.6rem", fontWeight: 700, padding: "0.1rem 0.4rem", background: "var(--accent-dim)", border: "1px solid var(--accent)", borderRadius: "2px", color: "var(--accent)" }}>Manche {ev.manche}</span>
                  </>
                )}
                {ev.inscrit && (
                  <span style={{ fontSize: "0.6rem", fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", padding: "0.1rem 0.4rem", background: "rgba(46,180,96,0.15)", border: "1px solid #2eb460", color: "#2eb460", borderRadius: "2px" }}>Inscrit ✓</span>
                )}
              </div>
