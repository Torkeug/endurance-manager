...
export default function AccueilDemo() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div className="card">
        <div style={labelStyle}>{t("myUpcomingEvents")}</div>
        {[
          ...
          <div key={ev.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.55rem 0", borderBottom: "1px solid var(--border)" }}>
            ...
            <button className="btn btn-secondary" style={{ fontSize: "0.78rem", padding: "0.25rem 0.6rem", opacity: 0.85 }}>{t("view")}</button>
          </div>
        ))}
      </div>
    </div>
