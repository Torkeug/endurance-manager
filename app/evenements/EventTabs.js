...
// Split into three functions so each part of the date block can be styled independently
// in the EventCard — day large, month/year small.
            ...
            <>
              <div
                ...
                style={{
                  fontSize: "1.4rem",
                  fontWeight: 500,
                  color: ev.archived ? "var(--text-dim)" : "var(--accent)",
                  lineHeight: 1,
                }}
              >
  ...
  return (
    <div
      ...
      style={{
        ...
        opacity: championship.archived ? 0.7 : 1,
      }}
    >
      <div
        style={{
      ...
      >
        <div>
          ...
          >
            {activeRounds.length} manche{activeRounds.length !== 1 ? "s" : ""}{" "}
            active{activeRounds.length !== 1 ? "s" : ""}
            {archivedRounds.length > 0 &&
              ` · ${archivedRounds.length} archivée${archivedRounds.length !== 1 ? "s" : ""}`}
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          {admin && (
            <>
              <Link
                href={`/championnats/${championship.id}/modifier`}
              ...
              <Link
                href={`/championnats/${championship.id}/nouveau-round`}
                onClick={(e) => e.stopPropagation()}
                className="btn btn-secondary btn-sm"
              >
                + Manche
...
export default function EventTabs({
  ...
  const [showArchivedChampionships, setShowArchivedChampionships] =
    useState(false);
            ...
            <>
              {activeChampionships.map((c) => (
                <ChampionshipSection
                  key={c.id}
                  championship={c}
                  rounds={championshipRounds.filter(
                    (r) => r.championship_id === c.id,
                  )}
                  admin={admin}
                />
              ))}
              ...
              {archivedChampionships.length > 0 && (
                <div style={{ marginTop: "1.5rem" }}>
                  <button
                    onClick={() =>
                      setShowArchivedChampionships(!showArchivedChampionships)
                    }
                    style={{
                      background: "none",
                      border: "none",
