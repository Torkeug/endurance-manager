...
export async function g61Fetch(url, token, driverId, refreshToken, fetchOptions = {}) {
  ...
  // Strip `next` so Next.js doesn't try to cache the response — our module-level
  // cache already handles TTL, and large responses (>2MB) silently fail in Next.js cache.
  const { next: _n, ...passThroughOptions } = fetchOptions;
  const res = await fetch(url, { ...passThroughOptions, headers: { Authorization: `Bearer ${token}` } });
  if (res.status === 401 && refreshToken) {
    const newToken = await tryRefreshToken(driverId, refreshToken);
    if (!newToken) return { ok: false, status: 401, data: null };
    const retry = await fetch(url, { ...passThroughOptions, headers: { Authorization: `Bearer ${newToken}` } });
    const retryData = retry.ok ? await retry.json() : null;
    if (ttl && retry.ok) setCached(cacheKey, retryData, ttl);
    // Return freshToken so callers can propagate it to subsequent g61Fetch calls
    // in the same request — avoids a second 401 when multiple calls share a stale token.
    return { ok: retry.ok, status: retry.status, data: retryData, freshToken: newToken };
  }
