...
export default function AvailabilityGrid({
  ...
  const getRowStyle = (slot) => {
    if (isRaceStart(slot)) return { background: 'rgba(201,168,76,0.10)', borderTop: '2px solid var(--accent)', borderBottom: '2px solid var(--accent)' }
  ...
  const getStickyBg = (slot) => {
    if (isRaceStart(slot)) return 'rgba(201,168,76,0.15)'
                      ...
                      <td style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '3px', flexWrap: 'nowrap' }}>
                          <span className="mono" style={{
                            fontSize: '0.8rem',
                            fontWeight: isRaceStart(slot) || isRaceEnd(slot) ? 700 : 400,
                            color: isRaceStart(slot) ? 'var(--accent)' : isRaceEnd(slot) ? 'var(--danger)' : 'var(--text)',
                          }}>
                            {formatTime(slot)}
                          </span>
                          {isRaceStart(slot) && (
                            <Badge label="▶" bg="var(--accent)" borderColor="var(--accent)" />
                          )}
                          {isRaceEnd(slot) && (
