...
function calculateAllStints(stints, teamEntry, driverPerf, igStartTime, igSunrise, igSunset) {
  ...
  if (result.length > 0 && raceEnd) {
    const lastRaceStint = result.find(s => s._irlEnd && s._irlEnd >= raceEnd)
      || result[result.length - 1] // fallback: last stint if none reach race end
    ...
    if (lastRaceStint._irlStart) {
      ...
      lastRaceStint._adjustedIrlEnd = raceEnd
      // Flag if the last stint doesn't reach race end (race not fully covered)
      lastRaceStint._doesNotCoverRaceEnd = lastRaceStint._irlEnd < raceEnd
    }
  }
...
export default function StintGrid({ teamEntryId, teamEntry, assignedDrivers }) {
  ...
  const projectedFinish = lastCalc?._isLastStint && lastCalc?._adjustedIrlEnd
    ? lastCalc._adjustedIrlEnd
    : lastCalc?._irlEnd
            ...
            {calculated.map((stint, i) => {
              ...
              return (
                <tr key={stint.id} style={{
                  ...
                  // Green left border marks the last stint covering race end
                  borderLeft: stint._isLastStint ? '3px solid #2eb460' : '3px solid transparent',
                }}>
                  ...
                  <td style={{ ...TD, textAlign: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.3rem' }}>
                      ...
                      {stint._isLastStint && stint._doesNotCoverRaceEnd && (
                        <span title="Ce relais ne couvre pas la fin de course"
