  ...
  if (result.length > 0 && raceEnd) {
    ...
    if (coveringStint) {
      ...
      if (coveringStint._irlStart && coveringStint._lapTimeSec) {
        ...
        // If user manually entered laps, respect that value (may exceed minimum).
        // Otherwise calculate minimum laps needed to cross the finish line.
        coveringStint._laps = coveringStint.laps_planned
          ? coveringStint.laps_planned
          : Math.ceil(remainingSecs / coveringStint._lapTimeSec)
