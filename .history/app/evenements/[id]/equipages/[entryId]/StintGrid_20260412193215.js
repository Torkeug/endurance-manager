    ...
    const coveringStint = result.find(s => s._irlEnd && s._irlEnd >= raceEnd)
      if (coveringStint) {
            ...
            if (coveringStint._irlStart && coveringStint._lapTimeSec) {
              ...
              // Minimum laps needed to reach race end — ceil ensures we cross the finish line.
              // Using the real lap count means FIN IRL will be slightly past race end,
              // which is consistent whether laps were auto-calculated or manually entered.
              coveringStint._laps = Math.ceil(remainingSecs / coveringStint._lapTimeSec)
