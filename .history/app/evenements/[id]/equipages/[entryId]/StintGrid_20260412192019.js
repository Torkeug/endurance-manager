    ...
    if (coveringStint) {
      ...
      if (coveringStint._irlStart) {
        ...
        if (coveringStint._lapTimeSec && coveringStint._adjustedDurationSec) {
          // Use Math.ceil so the last stint always ends AT or AFTER race end.
          // Math.floor could produce a lap count that finishes just before the line.
          coveringStint._laps = Math.ceil(coveringStint._adjustedDurationSec / coveringStint._lapTimeSec)
          // Sync _calcLaps so the laps input placeholder shows the same adjusted value
