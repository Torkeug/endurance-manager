...
// "1:52.345" or "112.345" → seconds (null if invalid)
function displayToSec(str) {
  if (!str || !str.trim()) return null
  const match = str.trim().match(/^(\d+):([0-5]\d)\.(\d{1,3})$/)
  if (!match) return null
  return parseInt(match[1]) * 60 + parseInt(match[2]) + parseFloat(`0.${match[3].padEnd(3, '0')}`)
}

// Auto Formatting
function formatLapInput(raw) {
  const digits = raw.replace(/\D/g, '').slice(0, 7) // max 7 digits
  if (digits.length === 0) return ''
  if (digits.length <= 3) return `0:00.${digits.padStart(3, '0')}`
  if (digits.length === 4) return `0:0${digits[0]}.${digits.slice(1)}`
...
function DriverRow({ signup, initialData, teamEntryId, onSaved }) {
  ...
  const hasData = initialData && (
    initialData.lap_time_dry || initialData.lap_time_wet ||
    initialData.fuel_dry     || initialData.fuel_wet
  )

  if (!editing) {
    return (
      <tr>
        <td style={{ fontWeight: 600, padding: '0.6rem 1rem' }}>{driver?.name || '—'}</td>
        <td style={tdStyle}>
          <span className="mono" style={{ color: hasData ? 'var(--text)' : 'var(--text-dim)' }}>
        ...
        <td style={tdStyle}>
          <span className="mono" style={{ color: hasData ? 'var(--accent)' : 'var(--text-dim)' }}>
        ...
        <td style={{ ...tdStyle, fontSize: '0.8rem', color: 'var(--text-dim)', maxWidth: '180px' }}>
    ...
    <>
      <tr style={{ background: 'var(--surface-2)' }}>
        <td colSpan={7} style={{ padding: '1rem' }}>
          <div style={{ fontWeight: 700, marginBottom: '1rem', color: 'var(--accent)' }}>
            {driver?.name}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem', marginBottom: '1.25rem' }}>
              ...
              <div className="form-grid">
                <div className="form-group">
                  <label>Chrono (MM:SS.mmm)</label>
                    <input type="text" value={form.lap_time_dry} onChange={set('lap_time_dry')}
                    onBlur={e => {
                    const val = e.target.value.trim()
                    if (!val) return
                    const sec = displayToSec(val)
                    if (sec !== null) {
                        setForm(prev => ({ ...prev, lap_time_dry: secToDisplay(sec) }))
                    }
                    // If invalid, leave as-is so driver can correct it
                    }}
                    placeholder="ex : 1:52.345" />
                </div>
                <div className="form-group">
                  <label>Conso (L/tour)</label>
                  <input type="number" value={form.fuel_dry} onChange={set('fuel_dry')}
                    placeholder="ex : 3.250" min="0" max="20" step="0.001" />
                </div>
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  <label>Réglages (informatif)</label>
                  <input type="text" value={form.setup_notes_dry} onChange={set('setup_notes_dry')}
            ...
            <div>
              ...
              <div className="form-grid">
                <div className="form-group">
                  <label>Chrono (MM:SS.mmm)</label>
                    <input type="text" value={form.lap_time_wet} onChange={set('lap_time_wet')}
                    onBlur={e => {
                    const val = e.target.value.trim()
                    if (!val) return
                    const sec = displayToSec(val)
                ...
                <div className="form-group">
                  <label>Conso (L/tour)</label>
                  <input type="number" value={form.fuel_wet} onChange={set('fuel_wet')}
                    placeholder="ex : 3.100" min="0" max="20" step="0.001" />
                </div>
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  <label>Réglages pluie (informatif)</label>
                  <input type="text" value={form.setup_notes_wet} onChange={set('setup_notes_wet')}
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            ...
            <button onClick={handleCancel} className="btn btn-secondary">Annuler</button>
          </div>
        </td>
      </tr>
    </>
...
export default function PerformanceData({ teamEntryId, assignedDrivers }) {
  ...
  useEffect(() => {
    if (!teamEntryId || assignedDrivers.length === 0) { setLoading(false); return }
    supabase.from('driver_performance')
      .select('*').eq('team_entry_id', teamEntryId)
      .then(({ data }) => {
        const map = {}
        ;(data || []).forEach(d => { map[d.driver_id] = d })
        setPerfData(map)
        setLoading(false)
      })
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr>
            ...
            <th style={thStyle}>Chrono sec ☀️</th>
            <th style={thStyle}>Chrono sec 💧</th>
            <th style={thStyle}>Conso sec ☀️</th>
            <th style={thStyle}>Conso sec 💧</th>
            <th style={thStyle}>Réglages sec</th>
            <th style={thStyle}></th>
          </tr>
        </thead>
        <tbody>
