...
export default async function VoitureDetail({ params }) {
  ...
  // Load signups for this event — both assigned to this entry and unassigned
  const { data: allSignups } = await supabase
    .from('signups')
    .select('*, drivers(id, name, irating), cars(name)')
  ...
  const infoItems = [
    ...
    { label: 'BOP Puissance',  value: entry.bop_power_percent ? `${entry.bop_power_percent}%` : '100%' },
    { label: 'BOP Poids',      value: entry.bop_weight_kg ? `${entry.bop_weight_kg}kg` : '0kg' },
