...
export default function ModifierEvenement({ params }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const irlStart = event.irl_start
        ? new Date(event.irl_start).toISOString().slice(0, 16)
        : ''
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      irl_start:      form.irl_start     || null,
      ig_start_time:  form.ig_start_time || null,
      notes:          form.notes.trim()  || null,
    }

    const { error: err } = await supabase
      .from('events').update(payload).eq('id', id)
  ...
  const handleDelete = async () => {
    if (!confirm('Supprimer cet événement ? Toutes les données associées (voitures, relais, disponibilités) seront supprimées.')) return
      ...
      <form onSubmit={handleSubmit}>
            ...
            <div className="form-group" style={{ gridColumn: '1 / -1' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                {DURATIONS.map(({ label, value }) => (
                  <button key={value} type="button" onClick={() => setDuration(value)} style={{
        ...
        <div className="card" style={{ marginBottom: '1.25rem' }}>
          ...
          <p style={{ fontSize: '0.8rem', color: 'var(--text-dim)', marginBottom: '1.25rem' }}>
            L&apos;heure IRL est l&apos;heure réelle de départ. L&apos;heure IG est l&apos;heure dans le jeu au moment du départ.
          </p>
          <div className="form-grid">
            <div className="form-group">
              <label htmlFor="irl_start">Départ IRL (date + heure)</label>
              <input id="irl_start" type="datetime-local" value={form.irl_start} onChange={set('irl_start')} />
            </div>
            <div className="form-group">
