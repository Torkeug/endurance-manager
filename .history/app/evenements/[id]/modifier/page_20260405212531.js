...
export default function ModifierEvenement({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const { error: err } = await supabase.from('events').update(payload).eq('id', params.id)

    if (err) {
      setError(err.message)
      setLoading(false)
    } else {
      router.push(`/evenements/${params.id}`)
      router.refresh()
    }
  }
  ...
  const handleDelete = async () => {
    ...
    const { error: err } = await supabase.from('events').delete().eq('id', params.id)
  ...
  return (
    <div className="page">
      <div className="page-header">
        ...
        <Link href={`/evenements/${params.id}`} className="btn btn-secondary">← Retour</Link>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'space-between', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            ...
            <Link href={`/evenements/${params.id}`} className="btn btn-secondary">Annuler</Link>
          </div>
