...
function SignupForm({ signup, carEntries, cars, startTimes, eventTimezone, eventId, driverId, onSaved, onCancel, onDelete }) {
  const supabase = createBrowserClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL,
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY
  )

  const [preferredClasses, setPreferredClasses]         = useState(signup?.preferred_class || [])
  const [preferredCarIds, setPreferredCarIds]           = useState(signup?.preferred_car_ids || [])
  const [preferredStartTimeIds, setPreferredStartTimeIds] = useState(signup?.preferred_start_time_ids || [])
  const [carEntryId, setCarEntryId]                     = useState(signup?.team_entry_id || '')
  const [notes, setNotes]                               = useState(signup?.notes || '')
  const [loading, setLoading]                           = useState(false)
  const [error, setError]                               = useState(null)

  const toggleClass = cls => setPreferredClasses(prev => prev.includes(cls) ? prev.filter(c => c !== cls) : [...prev, cls])
  const toggleCar   = cid => setPreferredCarIds(prev => prev.includes(cid) ? prev.filter(c => c !== cid) : [...prev, cid])
  const toggleST    = sid => setPreferredStartTimeIds(prev => prev.includes(sid) ? prev.filter(s => s !== sid) : [...prev, sid])
  ...
  const handleSubmit = async (e) => {
    ...
    let err
    if (signup?.id) {
      ({ error: err } = await supabase.from('signups').update(payload).eq('id', signup.id))
    } else {
  ...
  const handleDelete = async () => {
    if (!signup?.id) { onCancel(); return }
    if (!confirm('Se désinscrire ? Cette inscription sera supprimée.')) return
    const { error: err } = await supabase.from('signups').delete().eq('id', signup.id)
  ...
  return (
    <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {startTimes.map(st => (
              <label key={st.id} style={{
                ...
                <input type="checkbox" checked={preferredStartTimeIds.includes(st.id)}
                  onChange={() => toggleST(st.id)} style={{ accentColor: 'var(--accent)' }} />
                <div>
      ...
      <div className="card" style={{ marginBottom: '1.25rem' }}>
        ...
        {carEntries.length === 0 ? (
          <p style={{ color: 'var(--text-dim)', fontSize: '0.9rem' }}>Aucune équipe engagée pour cet événement.</p>
        ) : (
          <>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <label style={{
                ...
                border: '1px solid', borderColor: carEntryId === '' ? 'var(--accent)' : 'var(--border)',
                borderRadius: '3px', cursor: 'pointer', transition: 'all 0.15s',
              }}>
                <input type="radio" name="team_entry_id" value=""
                  checked={carEntryId === ''} onChange={() => setCarEntryId('')}
              ...
              {carEntries.map(entry => {
                ...
                return (
                  <label key={entry.id} style={{
                    ...
                    border: '1px solid', borderColor: isSelected ? 'var(--accent)' : 'var(--border)',
                    borderRadius: '3px', cursor: 'pointer', transition: 'all 0.15s',
                  }}>
                    <input type="radio" name="team_entry_id" value={entry.id}
                      checked={isSelected} onChange={() => setCarEntryId(entry.id)}
                      style={{ accentColor: 'var(--accent)' }} />
                    <div style={{ flex: 1 }}>
      ...
      <div className="card" style={{ marginBottom: '1.25rem' }}>
        ...
        {Object.keys(carsByClass).length === 0 ? (
          <p style={{ color: 'var(--text-dim)', fontSize: '0.85rem' }}>Aucune voiture disponible.</p>
        ) : (
      ...
      <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'space-between', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <button type="submit" className="btn btn-primary" disabled={loading}>
            {loading ? 'Enregistrement…' : signup?.id ? '✓ Mettre à jour' : "✓ S'inscrire"}
          </button>
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from('team_entries')
        .select('id, crew_name, class, car_id, start_time_id, cars(id, name, class)')
      ...
      <div className="card" style={{ marginBottom: '1.25rem' }}>
        ...
        <div className="form-group">
          ...
          <select id="driver_id" value={driverId}
            onChange={e => { setDriverId(e.target.value); setEditingSignup(null) }}>
        ...
        <>
          ...
          {existingSignups.length > 0 && (
            <div style={{ marginBottom: '1.25rem' }}>
              <h3 style={{ marginBottom: '0.75rem' }}>Inscriptions existantes</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    ...
                    {editingSignup?.id === signup.id ? (
                      <div className="card">
                        <h3 style={{ marginBottom: '1rem', color: 'var(--text-dim)' }}>
                          Modifier — {signup.team_entries?.crew_name || 'Sans équipe'}
                        </h3>
                        <SignupForm
                          signup={signup}
                      ...
                      <div className="card" style={{
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '0.8rem', color: 'var(--text-dim)', marginTop: '0.2rem' }}>
                            {signup.team_entries?.cars?.name && `${signup.team_entries.cars.name} · `}
                            {(signup.preferred_class || []).join(', ') || 'Pas de classe préférée'}
                          </div>
                          {signup.notes && (
                        ...
                        <button onClick={() => setEditingSignup(signup)}
          ...
          {editingSignup === 'new' ? (
            <div className="card" style={{ marginBottom: '1.25rem' }}>
              <h3 style={{ marginBottom: '1rem', color: 'var(--text-dim)' }}>Nouvelle inscription</h3>
              <SignupForm
                signup={null}
                carEntries={carEntries}
                cars={cars}
            ...
            <button onClick={() => setEditingSignup('new')} className="btn btn-primary">
              + {existingSignups.length > 0 ? 'Ajouter une inscription' : "S'inscrire"}
            </button>
          )}
        </>
      )}
