...
export default function Inscription({ params }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from('car_entries').select('id, crew_name, class, cars(name)')
  ...
  // When driver changes, load existing signup
  useEffect(() => {
    if (!driverId) { setExisting(null); return }
    supabase.from('signups').select('*, cars(name), car_entries(crew_name)')
  ...
  const handleSubmit = async (e) => {
    ...
    if (err) {
      ...
      const { data } = await supabase.from('signups')
        .select('*, cars(name), car_entries(crew_name)')
        .eq('event_id', id).eq('driver_id', driverId).single()
      setExisting(data)
    }
  }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="card" style={{ marginBottom: '1.25rem' }}>
          ...
          <p style={{ fontSize: '0.8rem', color: 'var(--text-dim)', marginBottom: '1.25rem' }}>
            Indiquez l&apos;équipe que vous souhaitez rejoindre pour cet événement.
          </p>

          {carEntries.length === 0 ? (
            <p style={{ color: 'var(--text-dim)', fontSize: '0.9rem' }}>
              Aucune équipe engagée pour cet événement pour l&apos;instant.
            </p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              {carEntries.map(entry => (
                <label key={entry.id} style={{
                  display: 'flex', alignItems: 'center', gap: '0.75rem',
                  padding: '0.75rem 1rem',
                  background: carEntryId === entry.id ? 'var(--accent-dim)' : 'var(--surface-2)',
                  border: '1px solid',
                  borderColor: carEntryId === entry.id ? 'var(--accent)' : 'var(--border)',
                  borderRadius: '3px', cursor: 'pointer', transition: 'all 0.15s',
                }}>
                  <input type="radio" name="car_entry_id" value={entry.id}
                    checked={carEntryId === entry.id}
