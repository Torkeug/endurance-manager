  ...
  const handleToggle = async () => {
    ...
    if (!archived) {
      ...
      const { data: event } = await supabase
        .from('events').select('circuit_id, circuits(name)').eq('id', eventId).single()
      ...
      const { data: teamEntries } = await supabase
        .from('team_entries').select('id, car_id, cars(name)').eq('event_id', eventId)
      if (teamEntries?.length > 0) {
        await Promise.all(teamEntries.map(te =>
          te.cars?.name
      ...
      const { data: signups } = await supabase
        .from('signups').select('id, driver_id, drivers(name)').eq('event_id', eventId)
      if (signups?.length > 0) {
      ...
      if (teamEntryIds.length > 0) {
        const { data: stints } = await supabase
          .from('stints').select('id, driver_id, drivers(name)')
          .in('team_entry_id', teamEntryIds)
        if (stints?.length > 0) {
          await Promise.all(stints.map(s =>
            s.drivers?.name
