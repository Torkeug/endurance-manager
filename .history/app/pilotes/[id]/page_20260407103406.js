                  ...
                  <div style={{ background: 'var(--surface-2)', borderRadius: '3px', padding: '0.75rem' }}>
                    ...
                    {teamEntry ? (
                      <>
                        ...
                        <Link href={`/evenements/${event?.id}/equipages/${teamEntry.id}`} className="btn btn-secondary btn-sm">
                            Équipage →
                        </Link>
                        <Link href={`/evenements/${event?.id}/equipages/${teamEntry.id}#relais`} className="btn btn-primary btn-sm">
                            Relais →
                        </Link>
                        </div>
                      </>
                    ...
                    <div style={{ background: 'var(--surface-2)', borderRadius: '3px', padding: '0.75rem' }}>
                      ...
                      {myStints.length > 0 ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                          {myStints.map(stint => (
                            <div key={stint.id} className="mono" style={{ fontSize: '0.78rem' }}>
                              <span style={{ color: 'var(--accent)' }}>#{stint.stint_number}</span>
                              {stint.rain && ' 💧'}
                              {stint.tyre_change && ' 🛞'}
                            </div>
                          ))}
