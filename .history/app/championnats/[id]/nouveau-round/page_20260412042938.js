...
export default function NouveauRound({ params }) {
  ...
  const [form, setForm] = useState({
    name: '', duration_minutes: '', circuit_id: '', format: '',
    ig_start_time: '', timezone: 'Europe/Paris', ig_sunrise: '', ig_sunset: '', notes: '',
  })
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from('events').select('id', { count: 'exact' }).eq('championship_id', id),
    ]).then(([{ data: champ }, { data: circuitsData }, { data: typesData }, { data: durData }, { count }]) => {
      setChampionship(champ)
      setCircuits(circuitsData || [])
      setEventTypes(typesData?.map(t => t.name) || [])
      setDurations(durData || [])
      setRoundCount(count || 0)
      // Pre-fill name with round number
      setForm(prev => ({ ...prev, name: `${champ?.name || ''} — Manche ${(count || 0) + 1}` }))
    })
  }, [id])
  ...
  if (!authChecked) return (
    <div className="page"><p style={{ color: 'var(--text-dim)' }}>Vérification des droits…</p></div>
  )
  ...
  return (
    <div className="page">
      ...
      <div style={{
        ...
        💡 Les horaires IRL de départ se configurent après création de la manche.
      </div>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
                {durations.map(({ minutes }) => (
                  <button key={minutes} type="button"
                    onClick={() => { setDuration(minutes); setCustomH(''); setCustomM('') }}
