...
export default function SettingsManager({ initialPresets, initialDefaultDuration, initialSpecialStartTimes }) {
  ...
  const [editStLabel, setEditStLabel]           = useState('')
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <React.Fragment key={st.id}>
                    ...
                    <td style={{ ...TD, fontWeight: 600, color: 'var(--accent)' }} className="mono">
                        {String(st.hour).padStart(2,'0')}:{String(st.minute).padStart(2,'0')}
                    </td>
                    <td style={TD}>{st.label || '—'}</td>
                    <td style={{ ...TD, textAlign: 'right' }}>
                        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                        <button onClick={() => { setEditingSpecialId(st.id); setEditStH(String(st.hour)); setEditStM(String(st.minute)); setEditStLabel(st.label || '') }}
                    ...
                    <tr>
                        ...
                        <div style={{ background: 'var(--surface-2)', padding: '1rem' }}>
                            ...
                            <input type="number" min="0" max="59" value={editStM}
                                ...
                                border: '1px solid var(--border)', borderRadius: '3px',
                                color: 'var(--text)', fontFamily: 'var(--font-mono), monospace', fontSize: '0.9rem' }} />
                            <input type="text" value={editStLabel}
                                onChange={e => setEditStLabel(e.target.value)} placeholder="Libellé (optionnel)"
                                style={{ padding: '0.45rem 0.5rem', background: 'var(--surface)',
                                border: '1px solid var(--border)', borderRadius: '3px',
                                color: 'var(--text)', fontSize: '0.9rem', minWidth: '180px' }} />
                            </div>
