...
export default function SettingsManager({
  ...
  const handleSaveDefault = async () => {
    ...
    setError(null);
    // Upsert by key — creates the row if it doesn't exist yet, updates if it does.
    // The settings table is a simple key/value store, not a typed config table.
    const { error: err } = await supabase
      .from("settings")
      .upsert({
  ...
  const handleEditSpecialTime = async () => {
    ...
    setSpecialTimes((prev) =>
      prev
        .map((s) => (s.id === editingSpecialId ? data : s))
        .sort(
          (a, b) =>
            DAY_ORDER[a.day_of_week] - DAY_ORDER[b.day_of_week] ||
            a.hour * 60 + a.minute - (b.hour * 60 + b.minute),
        ...
        {addingSpecial && (
          ...
          >
            ...
            >
              <select
                value={newStDay}
                onChange={(e) => setNewStDay(e.target.value)}
                style={{
                  padding: "0.45rem 0.5rem",
                  background: "var(--surface)",
                  border: "1px solid var(--border)",
                        ...
                        <td colSpan={2} style={{ padding: 0 }}>
                          ...
                          >
                            ...
                            >
                              <select
                                value={editStDay}
                                onChange={(e) => setEditStDay(e.target.value)}
                                style={{
                                  padding: "0.45rem 0.5rem",
                                  background: "var(--surface)",
                                  border: "1px solid var(--border)",
                              ...
                              <input
                                ...
                                max="23"
                                value={editStH}
                                onChange={(e) => setEditStH(e.target.value)}
                                placeholder="HH"
                                style={{
                                  width: "60px",
                                  padding: "0.45rem 0.5rem",
                              ...
                              <input
                                ...
                                max="59"
                                value={editStM}
                                onChange={(e) => setEditStM(e.target.value)}
                                placeholder="MM"
                                style={{
                                  width: "60px",
                                  padding: "0.45rem 0.5rem",
