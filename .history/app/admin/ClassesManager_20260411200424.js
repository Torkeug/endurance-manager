...
export default function ClassesManager({ initialClasses, initialCars }) {
  ...
  const getCarsInClass = (className) => cars.filter(c => c.class === className)
  const getUnclassedCars = () => cars.filter(c => !c.class || !classes.find(cl => cl.name === c.class))

  const assignCar = async (carId, className) => {
    const { data, error: err } = await supabase.from('cars')
      .update({ class: className }).eq('id', carId).select().single()
    if (err) {
      if (err.code === '23505') {
  ...
  const handleEdit = async () => {
    ...
    const { data, error: err } = await supabase.from('car_classes')
      ...
      if (err) {
        if (err.code === '23505') {
          setError('Ce nom de classe existe déjà.')
        } else {
          setError(err.message)
        }
        setSaving(false); return
    ...
    if (oldClass && oldClass.name !== newName.trim()) {
      await supabase.from('cars')
        .update({ class: newName.trim() }).eq('class', oldClass.name)
      setCars(prev => prev.map(c => c.class === oldClass.name ? { ...c, class: newName.trim() } : c))
    }
  ...
  const handleDelete = async (id, className) => {
    ...
    const msg = carsInClass.length > 0
    ...
    if (carsInClass.length > 0) {
      await supabase.from('cars').update({ class: null }).eq('class', className)
      setCars(prev => prev.map(c => c.class === className ? { ...c, class: null } : c))
    }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '0.75rem' }}>
          ...
          return (
            <div key={cls.id} className="card" style={{ padding: 0, overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.85rem 1rem', flexWrap: 'wrap' }}>
                <div style={{ flex: 1 }}>
                  <span className="badge badge-driver" style={{ fontSize: '0.9rem', padding: '0.25rem 0.6rem' }}>
                    {cls.name}
                  </span>
                  <span style={{ marginLeft: '0.75rem', fontSize: '0.8rem', color: 'var(--text-dim)' }}>
                    {carsInClass.length} voiture{carsInClass.length !== 1 ? 's' : ''}
                  </span>
                </div>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  <button onClick={() => handleDelete(cls.id, cls.name)} className="btn btn-danger btn-sm">Supprimer</button>
                </div>
              </div>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                        {carsInClass.map(car => (
                          <div key={car.id} style={{
                            ...
                            borderRadius: '3px', fontSize: '0.85rem', fontWeight: 600,
                          }}>
                            {car.name}
                            <button onClick={() => unassignCar(car.id)}
                              style={{ background: 'none', border: 'none', cursor: 'pointer',
                                color: 'var(--accent)', fontSize: '1rem', lineHeight: 1, padding: '0' }}
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                        {unclassedCars.map(car => (
                          <button key={car.id} onClick={() => assignCar(car.id, cls.name)}
                            style={{
                              padding: '0.4rem 0.8rem',
                              background: 'var(--surface)', border: '1px solid var(--border)',
                              borderRadius: '3px', color: 'var(--text-dim)', fontSize: '0.85rem',
        ...
        {classes.length === 0 && (
          <div className="card"><div className="empty">Aucune classe configurée.</div></div>
        )}
      </div>
      ...
      {unclassedCars.length > 0 && (
        <div className="card" style={{ marginBottom: '0.75rem', borderColor: 'var(--accent-dim)' }}>
          <div style={{ fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.1em',
            textTransform: 'uppercase', color: 'var(--text-dim)', marginBottom: '0.5rem' }}>
            ⚠️ {unclassedCars.length} voiture{unclassedCars.length > 1 ? 's' : ''} sans classe
          </div>
