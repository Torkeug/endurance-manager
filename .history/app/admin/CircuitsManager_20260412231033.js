...
export default function CircuitsManager({ initialCircuits }) {
  ...
  // Pit lane time is auto-filled in team entry forms from this value —
  // it's the transit time in seconds from pit entry to pit exit for this circuit
  const validate = () => {
    if (!form.name.trim()) {
  ...
  const handleDelete = async (id) => {
    ...
    const { error: err } = await supabase
      ...
      .eq("id", id);
    if (err) {
      // 23503 = FK violation — circuit is referenced by one or more events
      if (err.code === "23503") {
        setError(
