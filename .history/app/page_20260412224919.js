...
function Badge({ label, color = 'var(--accent)', bg = 'var(--accent-dim)', border = 'var(--accent)' }) {
  return (
    <span style={{
      fontSize: '0.7rem', fontWeight: 700, padding: '0.1rem 0.4rem',
      background: bg, border: `1px solid ${border}`,
      borderRadius: '3px', color, whiteSpace: 'nowrap',
    }}>
      {label}
    </span>
  )
...
export default async function HomePage() {
  ...
  const [
    ...
    // All non-archived events with start times, circuits, team entries
    ...
    // Driver's own signups with event and team details
    ...
    // Driver's own stints for "Mon prochain relais"
    ...
    // Admin: total active drivers
    admin ? supabase.from('drivers').select('*', { count: 'exact', head: true }).eq('active', true) : { count: 0 },

    // Admin: test drivers count
    admin ? supabase.from('drivers').select('*', { count: 'exact', head: true }).eq('test_driver', true).eq('approved', true) : { count: 0 },

    // Admin: drivers with overdue membership (not test drivers)
    admin ? supabase.from('drivers').select('*', { count: 'exact', head: true }).eq('membership_ok', false).eq('approved', true).eq('test_driver', false) : { count: 0 },

    // Admin: team entries for incomplete team widgets
    admin ? supabase.from('team_entries').select(`
  ...
  // Map championship id → name for badge display
  const championshipsMap = Object.fromEntries(
    (championships || []).map(c => [c.id, c.name])
  )

  // Build a set of archived championship IDs so we can exclude their rounds
  // from upcoming events and signups — rounds aren't archived individually
  ...
  // First filter: exclude rounds of archived championships
  // Second filter: only keep events that still have at least one future start time
  ...
  // Admin : Team entries with zero signups — no drivers have expressed interest at all
  const noDrivers = admin ? (teamEntries || []).filter(te => (te.signups || []).length === 0) : []

  // Admin : Team entries that have signups but no stints with a driver assigned yet
  // (stints may exist but all be unassigned — driver_id is null)
  const noStints = admin ? (teamEntries || []).filter(te =>
    (te.signups || []).length > 0 &&
  ...
  return (
    <div className="page">
      ...
      {/* ── Admin widgets: incomplete team entries ── */}
      {admin && (
        <>
          {noDrivers.length > 0 && (
                  ...
                  <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', marginBottom: '0.35rem' }}>
                    ...
                    {nextEvent.championship_id && (
                      <Badge
                        ...
                        color="var(--text-dim)" bg="var(--surface-2)" border="var(--border)"
                      />
                    )}
                  </div>
                )}
        ...
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
          {admin && (
            <>
              ...
              <Link href="/championnats/nouveau" className="btn btn-primary">+ Créer un championnat</Link>
            </>
          )}
